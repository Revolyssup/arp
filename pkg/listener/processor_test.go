		...
		Routes: []config.RouteConfig{
			{
				...
				Plugins:  []config.PluginConfig{{Name: "plugin1", Type: "type1", Config: map[string]interface{}{"key": "value"}}},
				Matches:  []config.Match{{Path: "/path1"}},
			},
			{
				Name:     "route2",
				Listener: "listener2",
				Upstream: &config.UpstreamConfig{Name: "upstream2"},
				Plugins:  []config.PluginConfig{{Name: "plugin2", Type: "type2", Config: map[string]interface{}{"key": "value"}}},
				Matches:  []config.Match{{Path: "/path2"}},
			},
		},
	...
	updatedConfig := config.Dynamic{
		Routes: []config.RouteConfig{
			...
			},
			{
				...
				Plugins:  []config.PluginConfig{{Name: "plugin3", Type: "type3", Config: map[string]interface{}{"key": "value"}}},
				Matches:  []config.Match{{Path: "/path3"}},
			},
		},
		Upstreams: []config.UpstreamConfig{
			{Name: "upstream1", Nodes: []config.Node{{URL: "http://localhost:8081"}}},
